...
class Auth
{
    ...
    public function __invoke($req, $res, $next)
    {
        ...
        if ($jwt && $user = AuthModel::loadUser($jwt->data->userId)) {
            ...
            if (!is_dir(ForumEnv::get('AMXXBG_ROOT').'amxxbg/lang/'.$user->prefs['language'])) {
                Prefs::setUser($user, ['language' => ForumSettings::get('language')]);
            }
            if (!file_exists(ForumEnv::get('AMXXBG_ROOT').'style/themes/'.$user->prefs['style'].'/style.css')) {
