...
use AMXXBG\Core\Interfaces\Response;
...
class View
{
    ...
    */
    public function __construct()
    {
        ...
        $this->twig = new Environment($this->loader, [
            'cache' => ForumEnv::get('AMXXBG_ROOT') . 'cache',
            'debug' => true,
        ]);
    ...
    *******************************************************************************/
    ...
    public function display($nested = true)
    {
        ...
        if (file_exists(ForumEnv::get('AMXXBG_ROOT').'style/themes/'.$style.'/base_admin.css')) {
            $admStyle = '<link rel="stylesheet" type="text/css" href="'.Url::base().'/style/themes/'.$style.'/base_admin.css" />';
        } else {
            $admStyle = '<link rel="stylesheet" type="text/css" href="'.Url::base().'/style/themes/AMXXBG/base_admin.css" />';
        }
    ...
    *******************************************************************************/
    ...
    {
        $dir = ForumEnv::get('AMXXBG_ROOT').'style/themes/'.$style.'/';
        if (!is_dir($dir)) {
        ...
        if (isset($vars['themeTemplates']) && $vars['themeTemplates'] == true) {
            $templatesDir = ForumEnv::get('AMXXBG_ROOT') . 'style/themes/'.$style.'/view';
        } else {
            $templatesDir = ForumEnv::get('AMXXBG_ROOT') . 'amxxbg/View/';
        }
    ...
    protected function getDefaultPageInfo()
    {
        ...
        if (!Cache::isCached('quickjump') && is_file(ForumEnv::get('FORUM_CONFIG_FILE'))) {
            Cache::store('quickjump', \AMXXBG\Model\Cache::quickjump());
        }

        $title = Container::get('forum_settings') ? ForumSettings::get('o_board_title') : 'AMXXBG';
        ...
        if (User::get() !== null) {
            if (User::isAdminMod()) {
                $data['has_reports'] = \AMXXBG\Model\Admin\Reports::hasReports();
            }
            // check db configured
            if (Database::getConfig()['username'] !== null) {
                ...
                // FIXME rebuild
                // Antispam feature
                $lang_antispam_questions = require ForumEnv::get('AMXXBG_ROOT').'amxxbg/lang/'.User::getPref('language').'/antispam.php';
        ...
        if (ForumEnv::get('AMXXBG_SHOW_INFO')) {
            $data['exec_info'] = \AMXXBG\Model\Debug::getInfo();
            if (ForumEnv::get('AMXXBG_SHOW_QUERIES')) {
                $data['queries_info'] = \AMXXBG\Model\Debug::getQueries();
            }
        }
