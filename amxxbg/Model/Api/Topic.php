...
class Topic extends Api
{
    ...
    public function setupVariables($errors, $isAdmmod)
    {
        ...
        // Otherwise it should be in $amxxbg ($_pOST)
        else {
        ...
        if ($result) {
            ...
            foreach ($result as $curSubscriber) {
                ...
                if (!isset($notificationEmails[$curSubscriber['language']])) {
                    if (file_exists(ForumEnv::get('AMXXBG_ROOT').'amxxbg/lang/'.$curSubscriber['language'].'/mail_templates/new_topic.tpl')) {
                        // Load the "new topic" template
                        $mailTpl = trim(file_get_contents(ForumEnv::get('AMXXBG_ROOT').'amxxbg/lang/'.$curSubscriber['language'].'/mail_templates/new_topic.tpl'));

                        // Load the "new topic full" template (with post included)
                        $mailTplFull = trim(file_get_contents(ForumEnv::get('AMXXBG_ROOT').'amxxbg/lang/'.$curSubscriber['language'].'/mail_templates/new_topic_full.tpl'));
    ...
    public function insertReply($post, $tid, $curPosting, $isSubscribed)
    {
        ...
        \AMXXBG\Model\Forum::update($curPosting['id']);
