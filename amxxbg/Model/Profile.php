...
use AMXXBG\Core\Interfaces\Parser;
...
use AMXXBG\Model\Auth as AuthModel;
...
class Profile
{
        ...
        if (Input::query('key')) {
            ...
            if (Email::isBannedEmail($newEmail)) {
                if (ForumSettings::get('p_allow_banned_email') == '0') {
                    ...
                    $mailTpl = trim(file_get_contents(ForumEnv::get('AMXXBG_ROOT').'amxxbg/lang/'.User::getPref('language').'/mail_templates/banned_email_change.tpl'));
            ...
            if ($result) {
                if (ForumSettings::get('p_allow_dupe_email') == '0') {
                    ...
                    $mailTpl = trim(file_get_contents(ForumEnv::get('AMXXBG_ROOT').'amxxbg/lang/'.User::getPref('language').'/mail_templates/dupe_email_change.tpl'));
            ...
            $mailTpl = trim(file_get_contents(ForumEnv::get('AMXXBG_ROOT').'amxxbg/lang/'.User::getPref('language').'/mail_templates/activate_email.tpl'));
    ...
    public function uploadAvatar($id, $filesData)
    {
        ...
        if (is_uploaded_file($uploadedFile['tmp_name'])) {
            ...
            if (!@move_uploaded_file($uploadedFile['tmp_name'], ForumEnv::get('AMXXBG_ROOT').ForumSettings::get('o_avatars_dir').'/'.$id.'.tmp')) {
            ...
            list($width, $height, $type, ) = @getimagesize(ForumEnv::get('AMXXBG_ROOT').ForumSettings::get('o_avatars_dir').'/'.$id.'.tmp');
            ...
            if ($type == IMAGETYPE_GIF) {
                ...
                // Invalid type
                @unlink(ForumEnv::get('AMXXBG_ROOT').ForumSettings::get('o_avatars_dir').'/'.$id.'.tmp');
                throw new Error(__('Bad type'));
            }
            ...
            @rename(ForumEnv::get('AMXXBG_ROOT').ForumSettings::get('o_avatars_dir').'/'.$id.'.tmp', ForumEnv::get('AMXXBG_ROOT').ForumSettings::get('o_avatars_dir').'/'.$id.$extension);
            @chmod(ForumEnv::get('AMXXBG_ROOT').ForumSettings::get('o_avatars_dir').'/'.$id.$extension, 0644);
        } else {
    ...
    public function deleteAvatar($userId)
    {
        ...
        foreach ($filetypes as $curType) {
            if (file_exists(ForumEnv::get('AMXXBG_ROOT').ForumSettings::get('o_avatars_dir').'/'.$userId.'.'.$curType)) {
                @unlink(ForumEnv::get('AMXXBG_ROOT').ForumSettings::get('o_avatars_dir').'/'.$userId.'.'.$curType);
            }
        }
    }
    ...
    public function updateGroupMembership($id)
    {
        ...
        if ($oldGroupId == ForumEnv::get('AMXXBG_ADMIN') || $newGroupId == ForumEnv::get('AMXXBG_ADMIN')) {
            CacheInterface::store('admin_ids', Cache::getAdminIds());
        }

        // If the user was a moderator or an administrator, we remove him/her from the moderator list in all forums as well
        if ($newGroupId != ForumEnv::get('AMXXBG_ADMIN') && !Perms::getGroupPermissions($newGroupId, 'mod.is_mod')) {
    ...
    public function deleteUser($id)
    {
        ...
        if ($groupId == ForumEnv::get('AMXXBG_ADMIN')) {
            throw new Error(__('No delete admin message'));
        }

        if (Input::post('delete_user_comply')) {
            // If the user is a moderator or an administrator, we remove him/her from the moderator list in all forums as well
            if ($groupId == ForumEnv::get('AMXXBG_ADMIN') || Perms::getGroupPermissions($groupId, 'mod.is_mod')) {
        ...
        switch ($section) {
            ...
            case 'display':
            {
                ...
                if (Input::post('form_style')) {
                    $styles = \AMXXBG\Core\Lister::getStyles();
    ...
    public function groupList($user)
    {
        ...
        $result = DB::table('groups')
            ->selectMany($result['select'])
            ->whereNotEqual('g_id', ForumEnv::get('AMXXBG_GUEST'))
            ->orderBy('g_title');
    ...
    public function sendEmail($mail)
    {
        ...
        // Here we use strlen() not Utils::strlen() as we want to limit the post to AMXXBG_MAX_POSTSIZE bytes, not characters
        elseif (strlen($message) > ForumEnv::get('AMXXBG_MAX_POSTSIZE')) {
        ...
        $mailTpl = trim(file_get_contents(ForumEnv::get('AMXXBG_ROOT').'amxxbg/lang/'.User::getPref('language', $mail['recipient_id']).'/mail_templates/form_email.tpl'));
