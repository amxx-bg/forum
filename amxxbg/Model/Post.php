...
class Post
{
    ...
    public function checkErrorsPost($fid, $errors)
    {
        ...
        if (User::get()->is_guest) {
            ...
            $profile = new \AMXXBG\Model\Profile();
        ...
        if (strlen($message) > ForumEnv::get('AMXXBG_MAX_POSTSIZE')) {
            $errors[] = sprintf(__('Too long message'), Utils::forumNumberFormat(ForumEnv::get('AMXXBG_MAX_POSTSIZE')));
    ...
    public static function editPost($id, $canEditSubject, $post, $curPost, $isAdmmod, $username = null)
    {
        ...
        if ($canEditSubject) {
            ...
            \AMXXBG\Core\Search::updateSearchIndex('edit', $id, $post['message'], $post['subject']);
        } else {
            \AMXXBG\Core\Search::updateSearchIndex('edit', $id, $post['message']);
        }
    ...
    public function report($postId)
    {
        ...
        if (ForumSettings::get('o_report_method') == '1' || ForumSettings::get('o_report_method') == '2') {
            ...
            if (ForumSettings::get('o_mailing_list') != '') {
                ...
                $mailTpl = trim(file_get_contents(ForumEnv::get('AMXXBG_ROOT').'amxxbg/lang/'.User::getPref('language').'/mail_templates/new_report.tpl'));
    ...
    public function reply($post, $tid, $curPosting, $isSubscribed)
    {
        ...
        \AMXXBG\Core\Search::updateSearchIndex('post', $new['pid'], $post['message']);
        ...
        if ($result) {
            ...
            foreach ($result as $curSubscriber) {
                ...
                if (!isset($notificationEmails[$curSubscriber['prefs']['language']])) {
                    if (file_exists(ForumEnv::get('AMXXBG_ROOT').'amxxbg/lang/'.$curSubscriber['prefs']['language'].'/mail_templates/new_reply.tpl')) {
                        // Load the "new reply" template
                        $mailTpl = trim(file_get_contents(ForumEnv::get('AMXXBG_ROOT').'amxxbg/lang/'.$curSubscriber['prefs']['language'].'/mail_templates/new_reply.tpl'));
                        $mailTpl = Hooks::fire('model.post.send_notifications_reply_mail_tpl', $mailTpl);

                        // Load the "new reply full" template (with post included)
                        $mailTplFull = trim(file_get_contents(ForumEnv::get('AMXXBG_ROOT').'amxxbg/lang/'.$curSubscriber['prefs']['language'].'/mail_templates/new_reply_full.tpl'));
    ...
    public function insertTopic($post, $fid)
    {
        ...
        \AMXXBG\Core\Search::updateSearchIndex('post', $new['pid'], $post['message'], $post['subject']);
    ...
    public function sendNotificationsNewTopic($post, $curPosting, $newTid)
    {
        ...
        if ($result) {
            ...
            foreach ($result as $curSubscriber) {
                ...
                // Is the subscription email for User::getPref('language', $curSubscriber['id']) cached or not?
                if (!isset($notificationEmails[$curSubscriber['prefs']['language']])) {
                    if (file_exists(ForumEnv::get('AMXXBG_ROOT').'amxxbg/lang/'.$curSubscriber['prefs']['language'].'/mail_templates/new_topic.tpl')) {
                        // Load the "new topic" template
                        $mailTpl = trim(file_get_contents(ForumEnv::get('AMXXBG_ROOT').'amxxbg/lang/'.$curSubscriber['prefs']['language'].'/mail_templates/new_topic.tpl'));
                        ...
                        $mailTplFull = trim(file_get_contents(ForumEnv::get('AMXXBG_ROOT').'amxxbg/lang/'.$curSubscriber['prefs']['language'].'/mail_templates/new_topic_full.tpl'));
    ...
    public static function warnBannedUser($post, $newPost)
    {
        ...
        $mailTpl = trim(file_get_contents(ForumEnv::get('AMXXBG_ROOT').'amxxbg/lang/'.User::getPref('language').'/mail_templates/banned_email_post.tpl'));
