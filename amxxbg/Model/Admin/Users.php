...
class Users
{
    ...
    public function moveUsers()
    {
        ...
        $isAdmin = DB::table('users')->whereIn('id', $move['user_ids'])
                        ->where('group_id', ForumEnv::get('AMXXBG_ADMIN'))
                        ->findOne();
        if ($isAdmin) {
        ...
        $whereNotIn = [ForumEnv::get('AMXXBG_GUEST'), ForumEnv::get('AMXXBG_ADMIN')];
        ...
        if (Input::post('move_users_comply')) {
            ...
            if (!empty($userGroups) && $newGroup != ForumEnv::get('AMXXBG_ADMIN') && !$newGroupMod) {
    ...
    public function deleteUsers()
    {
        ...
        if (Input::post('delete_users_comply')) {
                ...
                if ($result) {
                    foreach ($result as $curPost) {
                        ...
                        if ($result2 == $curPost['id']) {
                            \AMXXBG\Model\Topic::delete($curPost['topic_id']);
                        } else {
                            \AMXXBG\Model\Post::delete($curPost['id'], $curPost['topic_id']);
                        }

                        \AMXXBG\Model\Forum::update($curPost['forum_id']);
                    }
                }
            } else {
            ...
            // Delete user avatars
            $userProfile = new \AMXXBG\Model\Profile();
    ...
    public function printUsers($conditions, $orderBy, $direction, $startFrom)
    {
        ...
        if ($result) {
            foreach ($result as $curUser) {
                ...
                if (($curUser['g_id'] == '' || $curUser['g_id'] == ForumEnv::get('AMXXBG_UNVERIFIED')) && $curUser['user_title'] != __('Banned')) {
