...
class Groups
{
    ...
    public function getGroupList($groups, $group)
    {
        ...
        foreach ($groups as $curGroup) {
            if (($curGroup['g_id'] != $group['info']['g_id'] || $group['mode'] == 'add') && $curGroup['g_id'] != ForumEnv::get('AMXXBG_ADMIN') && $curGroup['g_id'] != ForumEnv::get('AMXXBG_GUEST')) {
    ...
    public function groupListDelete($groupId)
    {
        ...
        foreach ($result as $curGroup) {
            if ($curGroup['g_id'] == ForumEnv::get('AMXXBG_MEMBER')) {
    ...
    public function addEdit($groups)
    {
        ...
        $isAdminGroup = (Input::post('group_id') && Input::post('group_id') == ForumEnv::get('AMXXBG_ADMIN')) ? true : false;
        ...
        if (Input::post('promote_next_group') &&
                ...
                !in_array(Input::post('promote_next_group'), [ForumEnv::get('AMXXBG_ADMIN'), ForumEnv::get('AMXXBG_GUEST')]) &&
    ...
    public function setDefaultGroup($groups)
    {
        ...
        if ($groupId == ForumEnv::get('AMXXBG_ADMIN') || $groupId == ForumEnv::get('AMXXBG_GUEST')) {
            throw new Error(__('Bad request'), 404);
        }
