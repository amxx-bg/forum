    ...
    public function getForumActions($forumId, $forumUrl, $isSubscribed)
    {
        ...
        if (!User::get()->is_guest) {
            if (ForumSettings::get('o_forum_subscriptions') == 1) {
                if ($isSubscribed) {
                    $forumActions[] = '<div class="alert alert-info" role="alert">'.__('Is subscribed').' </div><a class="btn btn-secondary" href="'.Router::pathFor('unsubscribeForum', ['id' => $forumId, 'name' => $forumUrl]).'">'.__('Unsubscribe').'</a>';
                } else {
                    $forumActions[] = '<a class="btn btn-secondary" href="'.Router::pathFor('subscribeForum', ['id' => $forumId, 'name' => $forumUrl]).'">'.__('Subscribe').'</a>';
                }
            }

            $forumActions[] = '<a class="btn btn-secondary" href="'.Router::pathFor('markForumRead', ['id' => $forumId, 'name' => $forumUrl]).'">'.__('Mark forum read').'</a>';
        }
