...
class Forum
{
    ...
    {
        $this->model = new \AMXXBG\Model\Forum();
        Lang::load('forum');
        Lang::load('misc');
    }
    ...
    public function display($req, $res, $args)
    {
        ...
        if (($curForum['post_topics'] == '' && User::can('topic.post')) || $curForum['post_topics'] == '1' || $isAdmmod) {
            $postLink = "\t\t\t".'<a class="btn btn-primary" href="'.Router::pathFor('newTopic', ['fid' => $args['id']]).'">'.__('Post topic').'</a></p>'."\n";
        } else {
            $postLink = '';
        }
    ...
    public function dealposts($req, $res, $args)
    {
        ...
        $topicModel = new \AMXXBG\Model\Topic();
