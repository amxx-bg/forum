...
class Topic extends Api
{
    ...
    {
        $this->model = new \AMXXBG\Model\Api\Topic();
    }
    ...
    public function newTopic($req, $res, $args)
    {
        ...
        if (empty($errors)) {
            ...
            if ($this->user->is_guest && isset($errors['banned_email']) && ForumSettings::get('o_mailing_list') != '') {
                \AMXXBG\Model\Post::warnBannedUser($post, $new['pid']);
            }
    ...
    public function newReply($req, $res, $args)
    {
        ...
        if (isset($args['qid'])) {
            $post['message'] = \AMXXBG\Model\Post::getQuote($args['qid'], $args['id']).$post['message'];
        }
        ...
        if (empty($errors)) {
            ...
            if (ForumSettings::get('o_topic_subscriptions') == '1') {
                \AMXXBG\Model\Post::sendNotificationsReply($args['id'], $curPosting, $new['pid'], $post);
            }
